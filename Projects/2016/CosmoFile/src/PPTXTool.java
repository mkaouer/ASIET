...
public class PPTXTool {
	String [] SupportedFormat = {".pdf",".html",".JPEG"};
	
	//Methods
	public void ConvertPPTXToPDF(String Path,String Target){
		try{
	    ...
	    PdfWriter pdfWriter = PdfWriter.getInstance(pdfDocument, new FileOutputStream(Target+"/"+file.getName().replace(new GetFileExtension().getExtension(file.getName()),".pdf")));
	...
	void ConvertPPTXToHTML(String Path,String Target){
		try{
	...
	void ConvertThisPptxTo(String Path,String Target,String Extension){
		if(Extension.equalsIgnoreCase(".pdf")){ConvertPPTXToPDF(Path,Target);}
		
		if(Extension.equalsIgnoreCase(".JPEG")){ConvertPPTtoPic(Path,Target);}
		if(Extension.equalsIgnoreCase(".html")){ConvertPPTXToHTML(Path,Target);}
